  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <input
        ...
        className="w-full rounded-lg border border-slate-700 bg-slate-800/60 py-2 pl-8 pr-3 text-sm text-white placeholder:text-slate-500 transition focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
      />

      {isOpen && normalizedTerm && (
        <div className="absolute left-0 right-0 z-30 mt-2 overflow-hidden rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
          {results.length > 0 ? (
            results.map((page) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-start gap-3 px-3 py-2.5 text-left transition hover:bg-slate-800"
                >
                  <Icon className="mt-0.5 h-4 w-4 flex-shrink-0 text-emerald-400" aria-hidden="true" />
                  <span>
                    <span className="block text-sm font-medium text-white">{page.label}</span>
                    <span className="mt-0.5 block text-xs text-slate-500">{page.subtitle}</span>
                  </span>
                </button>
              )
            })
